...
- Лимиты входящих HTTP/gRPC операций, размеры буферов, размеры пулов, таймауты и retry/backoff
  заданы через типизированную конфигурацию и валидируются на старте.
- Нет неограниченных `go func()` в handler'ах, циклах, consumer'ах и retry-коде; fan-out ограничен
  worker pool, semaphore, `errgroup.SetLimit` или аналогичным механизмом.
- Метрики насыщения сервиса опубликованы: in-flight операции, заполнение буферов, отказы из-за лимитов,
  использование пулов БД/кэша и активные worker goroutines.
...
- Размеры пулов PostgreSQL и cache-зависимостей выбраны с учётом максимального числа реплик,
  чтобы HPA или ручное масштабирование не создавали лавину соединений.
